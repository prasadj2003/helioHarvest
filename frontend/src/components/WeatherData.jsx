...
const WeatherData = () => {
    ...
    return (
        <div className="p-6 bg-white rounded-lg shadow-md w-96">
            <h3 className="text-lg font-semibold text-gray-700 mb-4">Weather Data</h3>
            {error ? (
                <p className="text-red-500">{error}</p>
            ) : weather ? (
                <div className="flex flex-col space-y-4">
                    <div className="flex items-center space-x-4">
                        ...
                        <span className="text-xl font-bold">{weather.data.values.temperature}°C</span>
                    </div>
                    <div className="flex items-center space-x-4">
                        <Droplets className="text-blue-500" size={24} />
                        <span className="text-gray-700 font-medium">Humidity:</span>
                        <span className="text-xl font-bold">{weather.data.values.humidity}%</span>
                    </div>
                    <div className="flex items-center space-x-4">
                        <Cloud className="text-gray-500" size={24} />
                        <span className="text-gray-700 font-medium">Cloud Cover:</span>
                        <span className="text-xl font-bold">{weather.data.values.cloudCover} okta</span>
                    </div>
                    <div className="flex items-center space-x-4">
                        <Sun className="text-yellow-500" size={24} />
                        <span className="text-gray-700 font-medium">UV Index:</span>
                        <span className="text-xl font-bold">{weather.data.values.uvIndex}</span>
                    </div>
                    <div className="flex items-center space-x-4">
                        <Wind className="text-green-500" size={24} />
                        <span className="text-gray-700 font-medium">Wind Speed:</span>
                        <span className="text-xl font-bold">{weather.data.values.windSpeed} m/s</span>
                    </div>
                </div>
            ) : (
                <p>Loading weather data...</p>
